...
class(sash_raw$mortality_day) # This is a character variable!
...
sash_raw$mortality_day <- as.numeric(sash_raw$mortality_day)
...
# help us turning a variable into a factor. 
...
# You will need to use some operators to specify the condition within case_when() function.

sash_raw <- sash_raw %>% 
  mutate(agegroup = case_when(age < 50 ~ "< 50 years",
                              age >= 50 & age < 70 ~ "50-69",
                              age >=70 ~ "70+")) %>% 
...
sash_raw %>% filter(`Institute Abbreviation` %in% c("EDI", "GLA") & sex == "F") %>%
  select(agegroup, sex, mortality)
...
sash_trim <- sash_raw %>% select(Record_ID, age, agegroup, sex, mortality_day, mortality, LOS_total, hyponatraemia)

# Now examine this data for any problems with data types
sapply(sash_trim, class)
...
sash_trim %>% filter(mortality_day != LOS_total)
...
sash_trim <- sash_trim %>%
  mutate(mortality = case_when(mortality == "No" ~ 0,
                               mortality == "Yes" ~ 1),
         futime = case_when(mortality == 1 ~ mortality_day,
                            mortality == 0 ~ LOS_total))
...
sash_trim %>% select(agegroup, sex, hyponatraemia, mortality, futime) %>%
  tbl_summary(by = mortality) %>% add_p()
...
fit <- survfit(Surv(futime, mortality) ~ hyponatraemia, data = sash_trim)
ggsurvplot(fit, conf.int = TRUE, pval = TRUE, data = sash_trim,
           risk.table = TRUE, xlim = c(0, 100))
